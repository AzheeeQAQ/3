...
func StateCalculator(req gol.Request, res *gol.Response) [][]byte {
	...
	LocalMachine, _ := rpc.Dial("tcp", "0.0.0.0:8060")
	...
	for i := 0; i < n; i++ {
		...
		fmt.Println(address)
		disWorker, _ := rpc.Dial("tcp", address)
...
func sliceWorld(breakPoints []int, board [][]uint8) [][][]uint8 {
	...
	for i, _ := range worlds {
		if i == len(worlds)-1 {
			worldLen = (breakPoints[i+1]) - breakPoints[i] + 1
		} else {
...
func main() {
	...
	listener, _ := net.Listen("tcp", "3.85.103.108:"+*brokerAddr)
